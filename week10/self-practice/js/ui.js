...
function saveQuotes() {
  // Get the current quotes array and store it in localStorage
...
function loadQuotes() {
  // Load stored quotes from localStorage (if any)
  const data = localStorage.getItem('quotes');
  if (!data) return;

  const storedQuotes = JSON.parse(data);

  // Refill the quotes array and keep the nextId consistent
  storedQuotes.forEach((q) => addQuote(q.content, q.author));

  // Adjust nextId to continue correctly
  const maxId = storedQuotes.reduce((max, q) => Math.max(max, q.id), 0);
  if (maxId >= 1) {
    // `nextId` is declared in quote.js, so we modify it through window if exposed globally
